...
# ── Embeddable script (no iframe) ─────────────────────────────────────────────
# Serves the same UI as "/" but as a <script> you can drop into any site.
# Additive only: does not change any existing route.
from flask import Response

EMBED_MOUNT = "ilmodels-newsletter"
...
def embed_js():
    ...
    def absolutize(s):
        for q in ('"', "'", "`", "("):
            s = s.replace(q + "/api/", q + base + "/api/")
        s = s.replace('="/static/', '="' + base + "/static/")
        return s
  ...
  if (!m) {
    ...
    if (cs && cs.parentNode) cs.parentNode.insertBefore(m, cs.nextSibling);
  ...
  function runInline() {
    ...
    var wAdd = window.addEventListener.bind(window);
    function patch(orig) {
      return function (t, f, o) {
        if (t === "DOMContentLoaded" || t === "load") {
          try { typeof f === "function" ? f() : f && f.handleEvent && f.handleEvent(); }
  ...
  var pending = D.srcs.slice();
  function next() {
    if (!pending.length) return runInline();
  ...
  next();
"""
    out = "(function(){var D=" + json.dumps(payload) + ";" + runtime + "})();"
    return Response(out, mimetype="application/javascript",
